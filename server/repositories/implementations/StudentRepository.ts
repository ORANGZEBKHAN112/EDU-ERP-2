...
export class StudentRepository implements IStudentRepository {
  async getAll(campusIds?: number[], schoolId?: number, filterCampusId?: number, filterClassId?: number, search?: string): Promise<Student[]> {
    ...
    let query = `
      SELECT s.*, c.CampusName, cl.ClassName 
      FROM Students s
    ...
    if (filterClassId) {
      query += ' AND s.ClassId = @filterClassId';
      request.input('filterClassId', sql.Int, filterClassId);
    }

...
export class FeeService implements IFeeService {
  ...
  async getVouchers(ctx: RequestContext, month?: string) {
    return this.feeRepo.getVouchers(ctx.campusIds, ctx.schoolId, month);
  }

  async getConfigurations(ctx: RequestContext) {
    return this.feeRepo.getStructures(ctx.campusIds, ctx.schoolId);
  }

  async configure(ctx: RequestContext, data: any) {
    await this.feeRepo.upsertStructure({
      ...data,

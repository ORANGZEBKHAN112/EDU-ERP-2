...
export class StudentController {
  ...
  getStudents = async (req: any, res: Response, next: NextFunction) => {
    try {
      ...
      const filterClassId = req.query.classId ? parseInt(req.query.classId as string) : undefined;
      const search = req.query.search as string;
      const students = await this.studentService.getAllStudents(campusIds, schoolId, filterCampusId, filterClassId, search);
      res.json(students);
    } catch (err) {
      next(err);
    }
